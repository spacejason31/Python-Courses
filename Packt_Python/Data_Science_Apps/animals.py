...
class Herbivore:
    '''Generates a single animal with a random survival skill.
    Has the ability to breed and age'''
    mutation_drift = 1
    fertility_rate = 0.25
    max_age = 4
...
class Island:
    '''Creates a location with a group of Herbivores.
    Simulates the passage of time, with each time-step including breeding and statistic-generating processes for each animal'''
    stats = dict()
    animals = list()
    max_pop, year = 0, 0
    ...
    def _simulate(self):
        ...
        for animal in self.animals:
            ...
            if animal.age <= animal.max_age:
                new_animals.append(animal)      #animal continues to live
...
class HarshIsland(Island):
    '''Same as Island, except has harsh conditions within [e_min, e_max]
    Rabbits with survival skill below the condition die at the beginning
    of the epoch'''
